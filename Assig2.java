...
//Created 10/18/17, last modified 10/20/17
...
public class Assig2
{
	public static void main (String[] args) throws IOException
	{
		...
		Results tally = new Results("results.txt"); 
		...
		while(playAgain && realWord != null && scrambleWord != null) 
		{
			...
			if(realWord != null && scrambleWord != null) //if real/scrambled words are available
			{
				...
				while(guessLeft > 0 && !correct)
				{
					...
					{
						correct = true; 
						tally.won(); //increment number of wins/rounds using results object
				...
				if(!correct) //if user hasn't guessed correctly
				{
					...
					tally.lost(); //increment number of losses/rounds using results object
				}
		...
		else //if user hsa decided to end game
			System.out.println("Game over!"); 
		
		System.out.println(tally.toString()); 
		tally.save(); //writes data in Results object to associated file 
	}
}