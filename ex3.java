/* CS 0401 Fall 2017 Java Example 3
...
public class ex3
{
      public static void main (String [] args)
	  {
		  ...
		  double pied = Math.PI;
		  System.out.println("Pi as a float: " + pie);
		  System.out.println("Pi as a double: " + pied);

		  double value = 2.7182818;
		  double round2DigitA = value * 100 + 0.5 / 100;  // Our first attempt is
		  		// wrong for a couple of reasons.  First of all, the operations are
		  		// evaluated in the incorrect order, and secondly we do not actually
		  		// eliminate any digits -- in order to do this we must truncate at
		  		// some point.

		  double round2DigitB = (int) value * 100 + 0.5 / 100; // Our second attempt
		  		// is wrong because of precedence -- the division is performed before
		  		// the addition, which is not what we want.
		  
		  double round2DigitC = (int) (value * 100 + 0.5) / 100;  // In our third
		  		// attempt we have the formula correct, but still have an incorrect
		  		// answer due to integer division.

		  double round2DigitD = (int) (value * 100 + 0.5) / 100.0;  // Finally we
				// get the right answer.  With a function (discussed soon) we could
				// generalize this to round to an arbitrary number of digits.
		  ...
		  s1 = (short) i1;  s2 = (short) i2;  s3 = (short) i3; //casting
		  b1 = (byte) i1;  b2 = (byte) i2;  b3 = (byte) i3; //casting
		  ...
		  // Comparing postfix and prefix ++ operators.  Note that these differences only
		  // exhibit themselves when the ++ operation is done within the context of another
		  // operation (here it is assignment).
		  int i = 10, j = 10;
		  int m = i++;
		  int n = ++j;
